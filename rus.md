...
А использовать конструкторы без [строгого режима][6] могжет быть просто опасно. Если вызывающий забывает использовать `new`, ваш код *не использует строгий режим* или *не является ES6-классом* _[вздох]_, все что вы присваиваете через `this` будет засорять глобальное пространство имен. Не очень красиво, я считаю.
